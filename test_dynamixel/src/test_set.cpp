...
int main(int argc, char *argv[]) 
{
  ...
  node = rclcpp::Node::make_shared("find_dynamixel");
  ...
  while (index < BAUDRATE_NUM)
  {
    ...
    if (result == false)
    {
      RCLCPP_INFO(node->get_logger(),"%s", log);
      RCLCPP_INFO(node->get_logger(),"Failed to init");
    }
    else
    {
    ...
    {
     RCLCPP_INFO(node->get_logger(),"%s", log);
     RCLCPP_INFO(node->get_logger(),"Failed to scan");
    }
    else
    {
